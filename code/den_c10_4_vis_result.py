...
if __name__ == "__main__":
    ...
    print(images.size())
    print(labels)
    affine_params = np.load('../result/affine_params_random.npy')
    print(affine_params)
    origin_images = torchvision.utils.make_grid(images).numpy().transpose(1, 2, 0)
    ...
    for i in range(10):
        ...
        grid = torch.nn.functional.affine_grid(affine_param.repeat(images.size()[0], 1, 1), images.size())
        trans_data = torch.nn.functional.grid_sample(images, grid)
        # print(trans_data.size())
        trans_images = torchvision.utils.make_grid(trans_data).numpy().transpose(1, 2, 0)
